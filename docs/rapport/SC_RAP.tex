...
Pour les autres besoins de chiffrement moins sensibles\footnote{Comme chiffrer
en utilisant un algorithme symétrique et une clef générée par la carte ou
chiffrer avec une clef publique}, nous avons utilisé une application
java \og{}FaceCrypt \fg{} qui permet de réaliser les opérations de chiffrement
plus rapidement que la carte.

Pour gérer que l'utilisateur puisse gérer facilement nos outils de chiffrement
dans son compte Facebook, nous avons réalisé une extension \emph{Mozilla Firefox} (SSNExt).
Cela nous a permis ainsi d'intégrer des éléments appartenant à notre projet 
dans une page Facebook.

\subsection{FaceCrypt}
C'est une application gérant une partie du chiffrement du projet et 
faisant le relais entre l'extension Firefox et le composant SoftCard. 
...
groupe maitrisait. Nous l'avons préféré au C pour les raisons cités ci-dessus
...
fonctions de hachage. Plusieurs algorithmes peuvent être utilisées mais
pour le projet ce sont AES (CBC avec une clef de 256) et pour RSA ce sont
des clefs de 1024 bits qui sont utilisées.


\subsubsection{Communications sécurisées} % Pour Baptiste

\subsubsection{Base de données}
Pour manipuler ses amis, un utilisateur peut les placer dans des listes, 
celles-ci sont géré par FaceCrypt et l'extension \emph{via} une base de données
\texttt{sqlite}\footnote{C'est une base de données contenue dans un seul fichier
qui permet d'être utilisé dans des programmes sans avoir à embarquer une 
base de données traditionnelle suivant un modèle client-serveur}. 
...
à multiples endroits :
\begin{itemize}
    \item manipulation de la page Facebook, un certain nombre de choses que 
    nous pensions triviales à réaliser nous a finalement posé beaucoup de 
    problèmes du fait de la minutie des développeurs a empếcher les utilisateurs
    de \og{}scripter \fg{} leurs actions sur le réseau social.
